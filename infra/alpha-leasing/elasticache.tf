locals {
  redis_security_group_id            = "sg-03a62be71ffd6a301"
  redis_cloudshell_security_group_id = "sg-08059f0d796a1c786"
}

# ElastiCache Redis cluster - minimal configuration
resource "aws_elasticache_cluster" "agent_leasing" {
  cluster_id           = "alpha-agent-leasing"
  engine               = "redis"
  ...
  engine_version       = "7.1"
  port                 = 6379
  subnet_group_name    = "alpha-elasticache-subnet-group"
  security_group_ids   = [local.redis_security_group_id, local.redis_cloudshell_security_group_id]
