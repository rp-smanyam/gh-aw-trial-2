module "leasing" {
  ...
  host_header_values = ["alpha-agent-leasing.knocktest.com"]
  ...
  additional_github_deploy_permissions = [
    # MCP service permissions
    {
      ...
      Resource = [
        "arn:aws:ecs:us-east-1:${data.aws_caller_identity.current.account_id}:service/knock_alpha_rp_ai/alpha-agent-leasing-mcp"
      ]
    },
    {
      Effect = "Allow"
      Action = ["iam:PassRole"]
      Resource = [
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/alpha-agent-leasing-mcp-task-role",
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/alpha-agent-leasing-mcp-execution-role"
      ]
    },
    # Voice service permissions
    {
      ...
      Action = ["ecs:UpdateService", "ecs:DescribeServices"]
      Resource = [
        "arn:aws:ecs:us-east-1:${data.aws_caller_identity.current.account_id}:service/knock_alpha_rp_ai/alpha-agent-leasing-voice"
      ]
    },
    {
      Effect = "Allow"
      Action = ["iam:PassRole"]
      Resource = [
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/alpha-agent-leasing-voice-task-role",
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/alpha-agent-leasing-voice-execution-role"
      ]
    },
