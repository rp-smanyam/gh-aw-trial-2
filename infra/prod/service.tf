module "leasing" {
  ...
  host_header_values = ["agent-leasing.knockcrm.com"]
  ...
  additional_github_deploy_permissions = [
    # Voice service permissions
    {
      ...
      Resource = [
        "arn:aws:ecs:us-east-1:${data.aws_caller_identity.current.account_id}:service/${var.cluster}/prod-agent-leasing-voice"
      ]
    },
    {
      Effect = "Allow"
      Action = ["iam:PassRole"]
      Resource = [
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/prod-agent-leasing-voice-task-role",
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/prod-agent-leasing-voice-execution-role"
      ]
    },
