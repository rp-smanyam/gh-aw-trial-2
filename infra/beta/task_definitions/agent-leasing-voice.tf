resource "local_file" "agent_leasing_voice" {
  filename = "agent-leasing-voice.json"
  ...
  content = jsonencode({
    containerDefinitions = [
      {
        ...
        logConfiguration = {
          logDriver = "awslogs"
          options = {
            awslogs-group         = "/ecs/${var.environment}-${var.application}-voice"
    ...
    family           = "${var.environment}-${var.application}-voice"
    networkMode      = "bridge"
    taskRoleArn      = "arn:aws:iam::${var.aws_account}:role/${var.environment}-${var.application}-voice-task-role"
    executionRoleArn = "arn:aws:iam::${var.aws_account}:role/${var.environment}-${var.application}-voice-execution-role"
  })
}
